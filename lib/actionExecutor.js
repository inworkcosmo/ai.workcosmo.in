...
async function executeAction(action, params, ctx) {
  ...
  switch (action) {
    ...
    case 'save_memory': {
      const { content, category = 'fact' } = params;
      const { saveMemory } = require('./memoryManager');
      const memory = await saveMemory(userId, companyId, content, category, 'auto');
      return {
        success: true,
        summary: memory.duplicated ? `Memory already exists: "${content}"` : `Saved memory: "${content}"`,
        id: memory.id
      };
    }
    ...
    case 'delete_memory': {
      ...
      const { deleteMemory } = require('./memoryManager');
      const result = await deleteMemory(id, userId);
      return {
        success: true,
        summary: `Forgot memory (ID: ${id})`,
        id: result.id
      };
    }
