...
async function getAuthedContext(req) {
  ...
  const requestedCompanyId = normalizeClientId(
    req.body?.companyId || req.query?.companyId || req.body?.clientId || user.companyId
  );
  ...
  if (!isAiModuleEnabled(company)) {
    const err = new Error('The AI module is not enabled for this workspace.');
    err.statusCode = 403;
    throw err;
  }
...
async function reserveAiCredit(companyId, userId, action, credits = 1) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const remaining = companyAiCreditsRemaining(companySnap.data() || {});
    if (remaining < credits) {
      const err = new Error('No AI credits remaining. Add credits in the Access Portal.');
      err.statusCode = 402;
      throw err;
    }
...
async function callHuggingFaceChat({ model, messages, temperature = 0.5, stream = false }) {
  ...
  const payload = await response.json().catch(() => ({}));
  return payload.choices?.[0]?.message?.content?.trim() || "I couldn't generate a response.";
}

// In-memory rate limiter (per-user, resets each minute)
...
function setCors(req, res) {
  ...
  const allowed = [/^https:\/\/.*\.workcosmo\.in$/, /^http:\/\/localhost(:\d+)?$/];
